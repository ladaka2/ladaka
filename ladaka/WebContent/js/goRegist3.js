$(document).ready(function() {
	...
	$('#selectCate').change(function() {
		$("#selectCate option:selected").each(function() {
			$("#str_cate").val($(this).text()); // 선택값 입력
			$("#str_cate").attr("disabled", true); // 비활성화
		});
	});
	...
	$("#registTrue").click(function() {
		...
		console.log(businessName + " / " + businessNum + " / " + password + " / " + email + " / " + businessKeyword);
		...
		if (password != passwordRe) {
			...
			if ((checkOne1.prop("checked") == true) && (checkOne2.prop("checked") == true)) {
				...
				var param = {};
				param = {};
				param.businessCate = businessCate;
				...
				param.email = email;
				param.businessKeyword = businessKeyword;

				$.ajax({
					type : "POST",
					url : "/ladaka/goRegistAjax3",
