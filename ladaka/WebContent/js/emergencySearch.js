...
function search() {
	...
	$.ajax({
		type : "POST",
		url : "/ladaka/emergencySearchAjax",
		data : param,
		dataType : "json",
		success : parseJson,
		error : function() {
...
function parseJson(data) {
	...
	$.each(data.result, function(index, entry) {
		...
		var no = index + 1;
		html += "<div class='emergencyList' x='" + entry["Y_POS"] + "' y='" + entry["X_POS"] + "' style='border-top:1px solid blue; border-bottom:1px solid blue; border-left:1px solid blue; border-right:1px solid blue; padding:10px;'>";
...
function initBtn() {// 버튼 연결
	$(".emergencyList").click(function() {
		$(".emergencyList").css("background-color", "#FFFFFF");
